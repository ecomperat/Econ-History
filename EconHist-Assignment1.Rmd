...
The $x_s$ should capture the the exposure to the stock market 
...
#In an ideal world, would'nt we be looking for household level data on stock market exposure?
...
#I would add that car sales effectively function as an instrumental variable
#for consumption, so the key assumption is that the decline in consumption during 
#the Great Depression is strongly predicted by car sales (exclusion restriction). But
# car sales is hardly the only channel through which consumption is affected, which
#brings us to the omitted variable bias you mentioned.
...
As indicated in $summary(re8_feols)$, both variables are removed because of collinearity.
...
proxy for consumption, we might think this regression is appropriate to capture the wealth channel of the crash, keeping in
mind we operate at the state level.
...
temporary increase in income uncertainty. According to this uncertainty hypothesis, there should be a
...
As we mentioned before, a potential problem is that only very few people paid income taxes and thus filed tax returns.
However, this problem could be mitigated by the fact that high incomes were more likely to report earnings accurately,
...
be appropriate for investigating an asset-based wealth channel by focusing on those who own these assets.
