...
/*======================================================================
	...
	Event study using monthly data around BVX Crisis List events
	
	
	Note: need to first run the code "create Figure 7 and Tables 3 4 and A10.do" before running this file
...
gen month_revised = 1
save "$root/data/temp/Timing of stocks and bonds - peak_B only", replace
...
save "$root/data/temp/master_data_monthly - with lags", replace
*/




use "$root/data/temp/master_data_monthly - with lags", clear
...
drop if year<1870
drop date
rename (year month) (year_revised month_revised)


merge 1:1 country year_revised month_revised using "$root/data/temp/Timing of stocks and bonds - peak_B only"


*** keep only events on the BVX list
keep if _merge==3
...
save  "$root/data/temp/tmp_event_all_MB", replace
...
foreach panel1 in "A" "B" "C" "D" "E" {
	...
	use "$root/data/temp/tmp_event_all_MB", clear
	...
	****** SUBSAMPLES **************	
	...
	graph export "$root/output/Figure 6`panel1'.pdf", replace
